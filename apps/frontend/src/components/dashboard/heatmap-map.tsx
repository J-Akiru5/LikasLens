  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-4 text-xs text-ink/60">
        ...
        {[
          { label: "Critical", color: URGENCY_COLORS.critical },
          { label: "High", color: URGENCY_COLORS.high },
          { label: "Medium", color: URGENCY_COLORS.medium },
          { label: "Low", color: URGENCY_COLORS.low },
        ].map((item) => (
        ...
        {!loading && !error && data && data.points.length === 0 && (
          <div className="absolute inset-0 z-[999] flex items-center justify-center pointer-events-none">
            <div className="bg-panel/90 backdrop-blur-sm rounded-2xl px-8 py-6 shadow-lg text-center">
              ...
              <p className="text-sm text-ink/60 font-medium">No reports found for this area</p>
              <p className="text-xs text-ink/40 mt-1">Try expanding the date range or clearing filters</p>
            </div>
          </div>
        )}
      </div>
      ...
      {selectedCluster && (
        <div className="bg-panel border border-ink/10 rounded-2xl p-5 shadow-sm">
          <div className="flex items-start justify-between mb-3">
            <h3 className="font-bold text-ink">Cluster Details</h3>
            <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-ink/50 text-xs uppercase tracking-wider">Location</span>
              <p className="text-ink font-medium mt-1">{selectedCluster.location}</p>
            </div>
            <div>
              <span className="text-ink/50 text-xs uppercase tracking-wider">Reports</span>
              <p className="text-ink font-bold text-xl mt-1">{selectedCluster.count}</p>
            </div>
            <div>
              <span className="text-ink/50 text-xs uppercase tracking-wider">Dominant Type</span>
              <p className="text-ink font-medium mt-1">{typeLabel(selectedCluster.dominant_type)}</p>
            </div>
            <div>
              <span className="text-ink/50 text-xs uppercase tracking-wider">Coordinates</span>
        ...
        <div className="bg-panel border border-amber/30 rounded-2xl p-5 shadow-sm">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-ink">Hot Zone Alert</h3>
            </div>
            <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              <span className="text-ink/50 text-xs uppercase tracking-wider">Reports (7 days)</span>
            ...
            <div>
              <span className="text-ink/50 text-xs uppercase tracking-wider">Urgency</span>
              <p
      ...
      {data && data.points.length > 0 && (
        <div className="flex flex-wrap gap-6 text-sm text-ink/60">
          ...
          <span>
            <strong className="text-ink">{data.clusters.length}</strong> cluster{data.clusters.length !== 1 ? "s" : ""}
          </span>
          <span>
            <strong className="text-ink">{data.hot_zones.length}</strong> hot zone{data.hot_zones.length !== 1 ? "s" : ""}
          </span>
        </div>
      )}
    </div>
