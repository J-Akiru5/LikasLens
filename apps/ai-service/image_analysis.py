...
# ---------------------------------------------------------------------------
# Environmental indicator mapping (expanded with heuristics)
# ---------------------------------------------------------------------------
ENVIRONMENTAL_INDICATORS: list[dict[str, Any]] = [
    {
        "classes": [39, 41, 44, 45, 46, 47, 48, 49, 52, 53, 54, 55],
        "label": "Litter / Waste",
        "type": "solid_waste",
        "severity": "medium",
...
def _merge_detections(
    ...
    env_labels = {d["label"] for d in env_detections}

    for env_det in env_detections:
        # Skip if COCO already detected the same label at similar location
        is_duplicate = False
        for coco_det in coco_detections:
            if coco_det["label"] == env_det["label"]:
                # Check bbox overlap (IoU > 0.5)
...
def classify_environmental_risk(detections: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    detected_labels = {d["label"].lower().replace(" ", "_") for d in detections}
    matched_indicators = []
        ...
        try:
            ...
            for result in env_results:
                ...
                for box, conf, cls_id in zip(result.boxes.xyxy, result.boxes.conf, result.boxes.cls):
                    ...
                    confidence = float(conf.item())
                    x1, y1, x2, y2 = [float(v) for v in box.tolist()]
                    # Get class name from model's names dict
