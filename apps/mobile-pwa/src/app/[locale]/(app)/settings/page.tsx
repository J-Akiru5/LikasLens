...
export default function SettingsPage() {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      showToast(ts("loggedOut"), "success");
      setTimeout(() => { window.location.href = "/login"; }, 500);
    } catch {
      showToast(ts("logoutFailed"), "error");
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (!user?.email) {
      showToast(ts("emailUnavailable"), "error");
      setActionLoading(null);
      return;
    }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await supabase.auth.signOut();
      localStorage.clear();
      showToast(ts("accountDeleted"), "success");
      setTimeout(() => { window.location.href = "/login"; }, 1000);
    } catch {
      showToast(ts("accountDeleteFailed"), "error");
      setActionLoading(null);
    }
  };
  ...
  return (
    <div className="min-h-full pb-24 bg-page">
      <header className="sticky top-0 z-30 bg-page/80 backdrop-blur-md border-b border-ink/10 px-4 h-16 flex items-center gap-2">
        <h1 className="ios-large-title ios-large-title--xl">{ts("settings")}</h1>
      </header>

      <main className="pb-6">
        {/* Platform */}
        <SectionTitle icon={Globe}>{ts("language")}</SectionTitle>
        ...
        <SectionTitle icon={Sun}>{ts("theme")}</SectionTitle>
        <div className="ios-grouped-list mx-4">
          {([
            { value: "civic" as const, label: ts("civic"), icon: Sun, desc: ts("civicDesc") },
            { value: "ghost" as const, label: ts("ghost"), icon: Moon, desc: ts("ghostDesc") },
          ]).map((opt) => {
            const Icon = opt.icon;
        ...
        {/* Notifications */}
        <SectionTitle icon={Bell}>{ts("notifications")}</SectionTitle>
        <div className="ios-grouped-list mx-4">
          <ToggleRow
            label={ts("criticalAlerts")}
            desc={ts("criticalAlertsDesc")}
            checked={prefs.criticalAlerts ?? true}
            onChange={(v) => updatePref("criticalAlerts", v)}
          />
          <ToggleRow
            label={ts("reportUpdates")}
            desc={ts("reportUpdatesDesc")}
            checked={prefs.reportUpdates ?? true}
            onChange={(v) => updatePref("reportUpdates", v)}
          />
          <ToggleRow
            label={ts("communityActivity")}
            desc={ts("communityActivityDesc")}
        ...
        {/* Privacy & Security */}
        <SectionTitle icon={Shield}>{ts("privacyDisplay")}</SectionTitle>
        <div className="ios-grouped-list mx-4">
          <ToggleRow
            label={ts("publicProfile")}
            desc={ts("publicProfileDesc")}
            checked={prefs.publicProfile ?? true}
            onChange={(v) => updatePref("publicProfile", v)}
          />
          <ToggleRow
            label={ts("showReportCount")}
            desc={ts("showReportCountDesc")}
            checked={prefs.showReportCount ?? true}
            onChange={(v) => updatePref("showReportCount", v)}
          />
          <ToggleRow
            label={ts("reducedMotion")}
            desc={ts("reducedMotionDesc")}
        ...
        {/* Account */}
        <SectionTitle icon={User}>{ts("account")}</SectionTitle>
        <div className="ios-grouped-list mx-4">
          {/* User info card */}
          {userInfo && (
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className="text-sm text-ink font-medium">{ts("changePassword")}</p>
              <p className="text-xs text-ink/50">{ts("sendResetLink")}</p>
            </div>
          ...
          >
            ...
            <p className="flex-1 text-left text-sm text-red-500 font-medium">{ts("deleteAccount")}</p>
          </button>
        </div>
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white border border-ink/10 shadow-2xl rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-ink/10">
              <h3 className="font-semibold text-lg text-ink">{ts("resetPassword")}</h3>
            ...
            <form onSubmit={handlePasswordChange} className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={actionLoading === "password"} className="flex-1 py-3 bg-green text-white text-sm font-medium rounded-xl disabled:opacity-50">
                  {actionLoading === "password" ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : ts("sendResetLink")}
                </button>
              </div>
            </form>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white border border-ink/10 shadow-2xl rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-ink/10">
              <h3 className="font-semibold text-lg text-ink">{ts("deleteAccount")}</h3>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {actionLoading === "delete" ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : ts("yesDelete")}
                </button>
              </div>
            </div>
          </div>
