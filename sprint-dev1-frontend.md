...
### #174 — Broken OG Image
- [x] **Fix Open Graph meta image for social sharing**
  - **Files:**
    - `apps/frontend/src/app/layout.tsx` (metadata)
    - `apps/mobile-pwa/src/app/layout.tsx` (metadata — added OG/Twitter)
    - `apps/admin-portal/src/app/layout.tsx` (metadata — added OG/Twitter)
    - `apps/*/src/app/twitter-image.jpg` (shared OG image across all apps)
  - **Description:** OG image did not render when sharing links on social media. Fixed by using `twitter-image.jpg` as the canonical OG image across all 3 apps, and adding proper `openGraph` + `twitter` metadata to mobile-pwa and admin-portal layouts.
  - **Est:** 1.5h
  - **Issue:** #174
...
- [ ] **Verify all loading.tsx and error.tsx files render correctly**
  - `apps/mobile-pwa/src/app/[locale]/(app)/error.tsx` (if exists)
  - `apps/mobile-pwa/src/app/[locale]/error.tsx` (if exists)
  - `apps/mobile-pwa/src/app/not-found.tsx`
  - Note: Most routes do NOT have `loading.tsx` files — consider adding them for key routes (dashboard, incidents, laws)
  - **Est:** 1.5h
