...
export default function ProfileSettingsPage() {
  ...
  const [currencyRate, setCurrencyRate] = useState<CurrencySetting | null>(
    null,
  );
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const res = await fetchEcoCreditRate<{
          success: boolean;
      ...
      <DashboardLayoutWrapper>
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pt-4">
          <div className="space-y-2">
            <div className="h-9 w-48 rounded-xl bg-ink/5 animate-shimmer" />
          </div>
    ...
    <DashboardLayoutWrapper greeting={displayName || "Citizen"}>
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-semibold tracking-tight text-3xl text-ink">
            Profile Settings
          </h1>
        </div>
            ...
            <div>
              ...
              {currencyRate && (
                <p className="font-mono text-xs text-ink/40 mt-2">
                  Eco-Credit Rate: 1 Eco = {currencyRate.currency_code}{" "}
                  {currencyRate.eco_credit_rate.toLocaleString(undefined, {
